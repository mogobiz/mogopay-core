function ProfileCtrl($scope, $location, $rootScope, $route) {
    ...
    //Main Variables
    $scope.minBirthDate = new Date().setFullYear(new Date().getFullYear() - 100);
    $scope.maxBirthDate = new Date().setFullYear(new Date().getFullYear() - 18);
    $scope.minEpiryDate = new Date();
    $scope.showUserUUID = false;
    $scope.userUUID = "";
    $scope.bithDateValue = $scope.maxBirthDate;
    if($rootScope.userProfile) {
        $scope.bithDateValue = new Date($rootScope.userProfile.account.birthDate);
