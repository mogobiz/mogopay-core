...
object Mogopay {
  ...
  class ShopTransactionStatusRef
      extends TypeReference[ShopTransactionStatus.type]
  ...
  case class AccountAddress(
      ...
      @JsonScalaEnumeration(classOf[CivilityRef]) civility: Option[
  ...
  case class Account(
      ...
      @JsonScalaEnumeration(classOf[RoleNameRef]) roles: List[
    ...
    private def hasRoleName(roleName: RoleName.RoleName): Boolean =
  ...
  object ShippingPriceError extends Enumeration {
    ...
    val INTERNATIONAL_SHIPPING_NOT_ALLOWED = Value(
  ...
  case class ShippingDataList(@JsonScalaEnumeration(
                                classOf[ShippingPriceErrorRef]) error: Option[
  ...
  case class ShippingCart(
      shippingPricesByShopId: Map[String, ShippingDataList]) {
    val hasError = shippingPricesByShopId.exists {
      _._2.hasError
    }
  }

  case class SelectShippingCart(
      shippingAddress: AccountAddress,
      shippingPriceByShopId: Map[String, ShippingData]) {
    val price = shippingPriceByShopId.map {
      _._2.price
    }.sum
  }

  case class ModificationStatus(
      uuid: String,
      xdate: java.util.Date,
      ipAddr: Option[String],
      @JsonScalaEnumeration(classOf[ShopTransactionStatusRef]) oldStatus: ShopTransactionStatus.ShopTransactionStatus,
      @JsonScalaEnumeration(classOf[ShopTransactionStatusRef]) newStatus: ShopTransactionStatus.ShopTransactionStatus,
      comment: Option[String],
      var dateCreated: Date = Calendar.getInstance().getTime,
      var lastUpdated: Date = Calendar.getInstance().getTime)

  case class BOTransactionLog(
      uuid: String,
      direction: String,
      log: String,
      provider: String,
      ...
      @JsonScalaEnumeration(classOf[TransactionShopStepRef]) step: TransactionShopStep.TransactionShopStep,
      var dateCreated: Date = Calendar.getInstance().getTime,
      var lastUpdated: Date = Calendar.getInstance().getTime)

  case class BOPaymentData(
      @JsonScalaEnumeration(classOf[PaymentTypeRef]) paymentType: PaymentType.PaymentType,
      @JsonScalaEnumeration(classOf[CBPaymentProviderRef]) cbProvider: CBPaymentProvider.CBPaymentProvider,
      transactionSequence: Option[String],
      orderDate: Option[java.util.Date],
      @JsonScalaEnumeration(classOf[ResponseCode3DSRef]) status3DS: Option[
        ResponseCode3DS.ResponseCode3DS],
      transactionId: Option[String],
      authorizationId: Option[String])

  case class BOCreditCard(
      number: String,
      holder: Option[String],
      expiryDate: java.util.Date,
      @JsonScalaEnumeration(classOf[CreditCardTypeRef]) cardType: CreditCardType.CreditCardType)

  case class TransactionUser(email: String,
                             amount: Long,
  ...
  case class BOShopTransaction(
      uuid: String,
      shopId: String,
      transactionUUID: String,
      amount: Long,
      currency: CartRate,
      @JsonScalaEnumeration(classOf[ShopTransactionStatusRef]) status: ShopTransactionStatus.ShopTransactionStatus,
  ...
  case class CancelResult(id: String,
                          status: PaymentStatus.PaymentStatus,
                          errorCodeOrigin: String,
                          errorMessageOrigin: Option[String])

  case class RefundResult(status: PaymentStatus.PaymentStatus,
                          errorCode: String,
  ...
  case class SessionData(uuid: String = java.util.UUID.randomUUID.toString,
                         ...
                         var selectShippingCart: Option[SelectShippingCart] =
                           None,
  ...
  case class ShippingParcel(height: Double,
                            width: Double,
                            length: Double,
                            weight: Double)
  ...
  case class CartWithShipping(count: Int,
    ...
    def this(cart: Cart, shopCarts: List[ShopCartWithShipping]) =
      this(
        cart.count,
        cart.rate,
        cart.price,
  ...
  case class ShopCartWithShipping(shopId: String,
    ...
    def this(shopCart: ShopCart, shippingPrice: Long) =
      this(
        shopCart.shopId,
        shopCart.rate,
        shopCart.price,
