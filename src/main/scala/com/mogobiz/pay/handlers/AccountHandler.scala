...
case class RequestPasswordChange(email: String,
                                 merchantId: String,
                                 passwordCB: String,
                                 isCustomer: Boolean)
...
case class AddressToAddFromGetParams(road: String,
                                     ...
                                     lphone: String) {
  def getAddress = {
    val telephone = telephoneHandler.buildTelephone(
      lphone,
      country,
...
case class SendNewPasswordParams(merchantId: String,
                                 email: String,
                                 locale: Option[String])

case class AssignBillingAddress(accountId: String,
                                address: AddressToAssignFromGetParams)

case class AddShippingAddress(accountId: String,
                              address: AddressToAddFromGetParams)

case class DeleteShippingAddress(accountId: String, addressId: String)

case class UpdateShippingAddress(accountId: String,
                                 address: AddressToUpdateFromGetParams)
...
case class PayPalParam(paypalUser: Option[String],
                       paypalPassword: Option[String],
                       paypalSignature: Option[String])
    extends CBParams

case class AuthorizeNetParam(apiLoginId: String, transactionKey: String)
    extends CBParams
...
case class AuthorizeNetParams(apiLoginID: String,
                              transactionKey: String,
                              md5Key: String)
    extends CBParams

case class CustomProviderParams(customProviderName: String,
                                customProviderData: String)
    extends CBParams

case class SystempayParams(systempayShopId: String,
...
class AccountHandler {
  ...
  def findByEmail(email: String,
                  merchantId: Option[String]): Option[Account] = {
  ...
  private def buildFindAccountRequest(
      email: String,
      merchantId: Option[String]): SearchDefinition = {
  ...
  def alreadyExistCompany(company: String,
                          merchantId: Option[String]): Boolean = {
    val req =
      search(Settings.Mogopay.EsIndex -> "Account") limit 1 from 0 query {
        boolQuery().must(
      ...
      .map { userAccount =>
        if (userAccount.loginFailedCount > MogopayConstant.MaxAttempts)
          ...
          val userAccountToIndex =
            userAccount.copy(loginFailedCount = 0,
                             lastLogin = Some(Calendar.getInstance().getTime))
  ...
  def login(email: String,
    ...
    val userAccountRequest = if (isCustomer) {
      ...
      val merchant = EsClient
        .search[Account](merchantReq)
    ...
    EsClient
      ...
      .map { userAccount =>
        if (userAccount.loginFailedCount > MogopayConstant.MaxAttempts)
          ...
          accountHandler.update(
            userAccount.copy(
              loginFailedCount = userAccount.loginFailedCount + 1),
            true)
  ...
  def id(seller: String): String = {
    ...
    this
      ...
      .map(_.uuid)
      .getOrElse(throw InvalidEmailException(s"$email"))
  }
  ...
  def findBySecret(secret: String): Option[Account] = {
    val req = search(Settings.Mogopay.EsIndex -> "Account") query termQuery(
      "secret",
      secret)
    EsClient.search[Account](req)
  }

  def save(account: Account, refresh: Boolean = true) =
    findByEmail(account.email, account.owner) match {
      case Some(_) =>
        throw AccountWithSameEmailAddressAlreadyExistsError(s"${account.email}")
      case None =>
  ...
  def update(account: Account, refresh: Boolean): Boolean = {
    ...
    EsClient.update[Account](Settings.Mogopay.EsIndex,
                             account,
                             upsert = false,
                             refresh = refresh)
  }
  ...
  def updatePassword(oldPassword: String,
                     password: String,
                     vendorId: String,
    ...
    val merchant =
      getMerchant(vendorId).getOrElse(throw VendorNotFoundException())
    val paymentConfig =
      merchant.paymentConfig.getOrElse(throw PaymentConfigNotFoundException())
    val pattern = paymentConfig.passwordPattern.getOrElse(
      throw PasswordPatternNotFoundException(""))
    val matching: Boolean = `match`(pattern, password)

    if (!matching) throw PasswordDoesNotMatchPatternException("")
    accountHandler.update(
      account.copy(password = new Sha256Hash(password).toHex),
      false)
  }

  def sendNewPassword(req: SendNewPasswordParams): Unit = {
    val account = findByEmail(req.email, Some(req.merchantId))
      .getOrElse(throw new UnauthorizedException(""))
    ...
    val newStatus =
      if (account.status == AccountStatus.WAITING_ENROLLMENT)
        AccountStatus.ACTIVE
      else account.status
    update(account.copy(loginFailedCount = 0,
                        status = newStatus,
                        password = new Sha256Hash(newPassword).toHex),
           refresh = true)
    notifyNewPassword(account, newPassword, req.locale)
  }

  def notifyNewPassword(account: Account,
                        newPassword: String,
                        locale: Option[String]) = {
    ...
    val data = s"""
         ...
         |"email" :"${account.email}",
         |"name" :"${account.firstName.getOrElse("")} ${account.lastName
                    .getOrElse("")}",
    ...
    val (subject, body) =
      templateHandler.mustache(vendor, "mail-new-password", locale, data)
    EmailHandler.Send(
      Mail(
        from = (senderEmail.getOrElse(vendor.get.email),
                senderName.getOrElse(
                  s"${vendor.get.firstName} ${vendor.get.lastName}")),
  ...
  def generateAndSendPincode3(uuid: String): Unit = {
    ...
    val newTelephone = phoneNumber.copy(status =
                                          TelephoneStatus.WAITING_ENROLLMENT,
                                        pinCode3 = Some(pinCode3))
    accountHandler.update(
      acc.copy(
        address = acc.address.map(_.copy(telephone = Option(newTelephone)))),
      false)
    ...
    if (emailType != EmailType.Signup) {
      ...
      if (currentDate - signupDate > Settings.Mail.MaxAge) {
        ...
        account.map { account =>
          ...
          val vendor = account.owner.flatMap(load)
          notifyAccountConfirmed(account,
                                 vendor,
  ...
  def generateNewSecret(accountId: String): Option[String] =
  ...
  def addCreditCard(accountId: String,
    ...
    case Some(cardId) =>
      updateCard(accountId, cardId, holder, expiryDate, ccType)
  }
  ...
  private def updateCard(accountId: String,
    ...
    val card = account.creditCards
      .find(_.uuid == ccId)
      .getOrElse(throw CreditCardDoesNotExistException(""))

    val newCard = card.copy(
      holder = holder,
      expiryDate = new Timestamp(
        new SimpleDateFormat("yyyy-MM").parse(expiryDate).getTime),
  ...
  private def createCard(accountId: String,
    ...
    val (hiddenN, cryptedN) = if (!UtilHandler.checkLuhn(number)) {
      ...
      (UtilHandler.hideCardNumber(number, "X"),
       SymmetricCrypt.encrypt(number, Settings.Mogopay.CardSecret, "AES"))
    }

    val expiryTime = new Timestamp(
      new SimpleDateFormat("yyyy-MM").parse(expiryDate).getTime)
  ...
  def getBillingAddress(accountId: String): Option[AccountAddress] =
    load(accountId).flatMap(_.address)
  ...
  def assignBillingAddress(accountId: String,
                           address: AddressToAssignFromGetParams): Unit = {
    load(accountId).map { account =>
      val newAddress = account.address match {
        case None => address.getAddress
        case Some(addr) => {
          val telephone =
            telephoneHandler.buildTelephone(address.lphone,
                                            address.country,
                                            TelephoneStatus.WAITING_ENROLLMENT)
      ...
      accountHandler.update(account.copy(address = Some(newAddress)),
                            refresh = true)
  ...
  def addShippingAddress(accountId: String,
                         address: AddressToAddFromGetParams): Unit =
    load(accountId).map { account =>
      val shippAddr = ShippingAddress(java.util.UUID.randomUUID().toString,
                                      active = true,
                                      address.getAddress)
  ...
  def updateShippingAddress(accountId: String,
                            address: AddressToUpdateFromGetParams): Unit =
    load(accountId).map { account =>
      account.shippingAddresses.find(_.uuid == address.id) map { addr =>
        val telephone =
          telephoneHandler.buildTelephone(address.lphone,
                                          ...
                                          TelephoneStatus.WAITING_ENROLLMENT)
        val newAddrs = account.shippingAddresses.filterNot(_ == addr) :+ addr
          .copy(
        ...
        accountHandler.update(account.copy(shippingAddresses = newAddrs),
                              refresh = true)
      }
    } getOrElse (throw AccountDoesNotExistException(s"$accountId"))
  ...
  def getActiveCountryStateShipping(
      accountId: String): Option[ActiveCountryStateShipping] = {
    load(accountId).map { account =>
      val addr = account.shippingAddresses
        .find(_.active)
        .map(_.address)
        .orElse(account.address)
      addr.map { addr =>
    ...
    load(accountId).map { account =>
      ...
      val cbParam = basePaymentProviderParam.map { ppp =>
        ...
        if (targetFile.exists) {
          val certificateData =
            scala.io.Source.fromFile(targetFile).getLines().mkString
      ...
      case Some(account) =>
        lazy val address = account.address.map { address =>
          ...
          address.copy(
            ...
            geoCoordinates = UtilHandler.computeGeoCoords(
              profile.billingAddress.road,
        ...
        val updateCBParam: Option[CBParams] =
          if (!profile.isMerchant)
            ...
            val cbProvider = CBPaymentProvider.withName(
              profile.cbProvider.getOrElse(throw new NoCBProviderSpecified))
            ...
            if (cbProvider == CBPaymentProvider.SIPS) {
              ...
              if (params.sipsMerchantParcomFileName.isDefined && params.sipsMerchantParcomFileName != Some(
                    "")) {
                val parcomTargetFile =
                  new File(dir, "parcom." + params.sipsMerchantId)
                if (params.sipsMerchantParcomFileContent
                      .getOrElse("")
                      .length > 0 && params.sipsMerchantParcomFileName
                      .getOrElse("")
                      .length > 0) {
                  parcomTargetFile.delete()
                  Files.write(parcomTargetFile.toPath,
                              params.sipsMerchantParcomFileContent.get
                ...
                try {
                  val oldSIPSMerchantParcomFileContent: Option[String] = (for {
                    ...
                    read[SIPSParams](cbp).sipsMerchantParcomFileContent).flatten
                  val oldSIPSMerchantParcomFileName: Option[String] = (for {
                    pc <- account.paymentConfig
                    cbp <- pc.cbParam
                  } yield
                    read[SIPSParams](cbp).sipsMerchantParcomFileName).flatten

                  params = params.copy(
                    sipsMerchantParcomFileContent =
                      oldSIPSMerchantParcomFileContent,
              ...
              params.sipsMerchantCertificateFileName match {
                case Some(a) if a.length > 0 =>
                  val certificateTargetFile =
                    new File(
                      dir,
                      "certif." + params.sipsMerchantCountry + "." + params.sipsMerchantId)
                  if (params.sipsMerchantCertificateFileContent
                        .getOrElse("")
                        .length > 0 && params.sipsMerchantCertificateFileName
                        .getOrElse("")
                        .length > 0) {
                    certificateTargetFile.delete()
                    Files.write(certificateTargetFile.toPath,
                                params.sipsMerchantCertificateFileContent.get
                                  .getBytes(StandardCharsets.UTF_8))
                  }

                  val targetFile = new File(dir, "pathfile")
                  val isJSP = params.sipsMerchantCertificateFileContent.exists(
                    _.indexOf("!jsp") > 0) ||
                    (targetFile.exists() && (new FileParamReader(
                  ...
                  if (isJSP)
                    certificateTargetFile.renameTo(
                      new File(certificateTargetFile.getAbsolutePath + ".jsp"))
                case _ =>
                  try {
                    val oldSIPSMerchantCertificateFileContent: Option[String] =
                      (for {
                        pc <- account.paymentConfig
                        cbp <- pc.cbParam
                      } yield
                        read[SIPSParams](cbp).sipsMerchantCertificateFileContent).flatten
                    val oldSIPSMerchantCertificateFileName: Option[String] =
                      (for {
                        pc <- account.paymentConfig
                        cbp <- pc.cbParam
                      } yield
                        read[SIPSParams](cbp).sipsMerchantCertificateFileName).flatten

                    params = params.copy(
                      sipsMerchantCertificateFileContent =
                        oldSIPSMerchantCertificateFileContent,
                      sipsMerchantCertificateFileName =
                        oldSIPSMerchantCertificateFileName
                    )
                  } catch {
                    case _: Throwable => params
                  }
        ...
        val paymentConfig =
          if (!profile.isMerchant) None
            ...
            val cbProvider = CBPaymentProvider.withName(
              profile.cbProvider.getOrElse(throw new NoCBProviderSpecified))
            val paymentMethod =
              CBPaymentMethod.withName(
                profile.paymentMethod.getOrElse(
                  throw new NoCBPaymentMethodSpecified))
            ...
            Some(
              PaymentConfig(
                ...
                paypalParam = profile.payPalParam.map(p =>
                  write(caseClassToMap(p))),
                applePayParam = profile.applePayParam.map(p =>
                  write(caseClassToMap(p))),
                cbParam = Some(write(updateCBParam)),
                emailField =
                  profile.emailField.filter(_ != "").getOrElse("user_email"),
        ...
        if ((profile.isMerchant && validateMerchantPhone) || (!profile.isMerchant && validateCustomerPhone)) {
          val lphone: Option[String] =
            newAccount.address.flatMap(_.telephone.map(_.lphone))
          val oldLPhone: Option[String] =
            account.address.flatMap(_.telephone.map(_.lphone))
        ...
        lazy val birthDate = try {
          ...
          case e: java.text.ParseException =>
            throw new InvalidDateFormatException(
              "Correct format: " + birthDayDateFormat.toPattern)
  ...
  def recycle() {
    val req = search(Settings.Mogopay.EsIndex -> "Account") query {
      boolQuery().must(
        ...
        rangeQuery("waitingEmailSince") gt 0 lte (System
          .currentTimeMillis() - Settings.AccountRecycleDuration)
      )
    }
      ...
      if (user.address.map(_.telephone.map(_.lphone)).flatten.nonEmpty &&
        ...
        load(accountId)
          .map { account =>
            ...
            if (account.address
                  .map(_.telephone.map(_.pinCode3))
                  .flatten == Some(encryptedCode) &&
                account.address.map(_.telephone.map(_.status)).flatten == Some(
                  TelephoneStatus.WAITING_ENROLLMENT)) {
              val newTel =
                Telephone("", lPhone, "", None, TelephoneStatus.ACTIVE)
              accountHandler.update(
                account.copy(address =
                  account.address.map(_.copy(telephone = Some(newTel)))),
                refresh = true)
            } else {
  ...
  def signup(signup: Signup): (Token, Account) = {
    val owner = if (signup.isMerchant) {
      ...
      Some(signup.vendor.getOrElse({
        val account =
          findByEmail(Settings.AccountValidateMerchantDefault, None).getOrElse {
    ...
    val countryCode =
      signup.address.country.getOrElse(
        throw InvalidInputException(
          s"Country not found. ${signup.address.country}"))
    ...
    def address(): AccountAddress = {
      ...
      val tel = telephoneHandler.buildTelephone(signup.lphone,
                                                country.code,
                                                phoneStatus)
    ...
    val account = Account(
      ...
      roles =
        List(if (signup.isMerchant) RoleName.MERCHANT else RoleName.CUSTOMER),
    ...
    if (needEmailValidation) {
      val validationUrl = signup.validationUrl + (if (signup.validationUrl
                                                        .indexOf("?") == -1) "?"
                                                  else
                                                    "&") + "token=" + URLEncoder
        .encode(token, "UTF-8")
        ...
        case Some(acc) =>
          ...
          val paymentConfig = merchant
            ...
            .getOrElse(throw new Exception(
              s"No payment config found found merchant $acc"))
          val paymentConfigSenderEmail =
            if (paymentConfig.senderEmail.getOrElse("").trim.length == 0) None
            else paymentConfig.senderEmail
          val paymentConfigSenderName =
            if (paymentConfig.senderName.getOrElse("").trim.length == 0) None
            else paymentConfig.senderName
          val senderEmail =
            paymentConfigSenderEmail.getOrElse(merchant.get.email)
          val senderName =
            paymentConfigSenderName.getOrElse(s"${merchant.get.firstName
              .getOrElse(senderEmail)} ${merchant.get.lastName.getOrElse("")}")
          notifyNewAccount(account,
                           vendor,
                           validationUrl,
                           token,
  ...
  def listMerchants(): Seq[(String, String)] = {
    ...
    EsClient
      ...
      .map(merchant =>
        (merchant.company.getOrElse(merchant.email), merchant.uuid))
  ...
  def notifyNewAccount(account: Account,
    ...
    val (subject, body) = templateHandler.mustache(
      vendor,
      "mail-signup-confirmation",
      locale,
      s"""
         |{
         |"templateImagesUrl": "${Settings.TemplateImagesUrl}",
...
object Token {
  ...
  def generateAndSaveToken(accountId: String,
                           tokenType: TokenType): Option[String] = {
  ...
  def generateToken(accountId: String,
                    owner: Option[String],
