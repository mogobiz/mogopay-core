...
class SystempayHandler(handlerName: String) extends PaymentHandler {
  ...
  def done(sessionData: SessionData, params: Map[String, String]): Uri = {
    ...
    val transaction: BOTransaction = EsClient.load[BOTransaction](Settings.Mogopay.EsIndex, transactionUUID).orNull // todo
  ...
  private def handleResponse(params: Map[String, String]): PaymentResult = {
    ...
    val vendor = EsClient.load[Account](Settings.Mogopay.EsIndex, vendorId).orNull // todo
    ...
    if (ok) {
      val transaction = EsClient.load[BOTransaction](Settings.Mogopay.EsIndex, transactionUUID).orNull // todo

      val botlog = BOTransactionLog(newUUID, "IN", params.mkString(", "), "SYSTEMPAY", transaction.uuid)
      EsClient.index(Settings.Mogopay.EsIndex, botlog, false) // todo
      ...
      EsClient.index(Settings.Mogopay.EsIndex, transaction.copy(creditCard = Some(creditCard)), false) // todo
