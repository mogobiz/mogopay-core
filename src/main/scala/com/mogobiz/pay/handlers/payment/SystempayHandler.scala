...
class SystempayHandler(handlerName: String) extends PaymentHandler {
  ...
  def startPayment(sessionData: SessionData): Either[String, Uri] = {
    val (transactionUUID, vendor, paymentConfig, paymentRequest) = getContext(
      sessionData)
    ...
    if (sessionData.mogopay) {
      val paymentResult = systempayClient.submit(
        this,
        sessionData,
        sessionData.uuid,
        vendor,
    ...
    } else if (Array(CBPaymentMethod.THREEDS_IF_AVAILABLE,
                     CBPaymentMethod.THREEDS_REQUIRED).contains(
                 paymentConfig.paymentMethod)) {
      threeDSResult = systempayClient.check3DSecure(
        sessionData,
        vendor,
        transactionUUID,
        paymentConfig,
      ...
      if (Option(threeDSResult)
            .map(_.code)
            .contains(ResponseCode3DS.APPROVED)) {
        ...
        Right(
          finishPayment(sessionData,
                        createThreeDSNotEnrolledResult(paymentRequest)))
      }
    } else {
      val paymentResult = systempayClient.submit(
        this,
  ...
  def validatePayment(transaction: BOTransaction,
                      amount: Long): Option[ValidatePaymentResult] = {
    //TODO à implémenter
    None
  }

  def refundPayment(transaction: BOTransaction,
                    amount: Long): Option[ValidatePaymentResult] = {
    //TODO à implémenter
    None
  }
  ...
  def done(sessionData: SessionData, params: Map[String, String]): Uri = {
    ...
    val resultatPaiement: PaymentResult =
      if (!Array(PAYMENT_CONFIRMED, PAYMENT_REFUSED).contains(
            transaction.status)) {
  ...
  def callbackPayment(sessionData: SessionData,
                      params: Map[String, String]): PaymentResult =
    handleResponse(sessionData,
                   params,
                   sessionData.locale,
                   TransactionStep.CALLBACK_PAYMENT)

  private def handleResponse(sessionData: SessionData,
                             params: Map[String, String],
                             locale: Option[String],
                             step: TransactionStep): PaymentResult = {
    val names: Seq[String] = params
      .filter({ case (k, v) => k.indexOf("vads_") == 0 })
      .keys
      .toList
      .sorted
    ...
    val parametresProvider: Map[String, String] = parse(
      org.json4s.StringInput(
        vendor.paymentConfig.map(_.cbParam).flatten.getOrElse("{}")))
      .extract[Map[String, String]]
    val ok: Boolean =
      SystempayUtilities.checkSignature(
        params("signature"),
        parametresProvider("systempayCertificate"),
        values.asJava)
    if (ok) {
      val transaction = boTransactionHandler.find(transactionUUID).orNull
      ...
      val vads_card_brand = try {
        CreditCardType.withName(
          params.getOrElse("vads_card_brand", CreditCardType.CB.toString))
      } catch {
        case NonFatal(e) =>
          CreditCardType.CB
      }
      ...
      var pr = PaymentResult(
        params.getOrElse("vads_sequence_number", "0"),
      ...
      if (month.nonEmpty && year.nonEmpty) {
        ...
        pr =
          pr.copy(expirationDate = simpleDateFormat.parse(month.get + year.get))
      }
      ...
      pr = pr.copy(
        ...
        status =
          if (params("vads_result") == "00") PaymentStatus.COMPLETE
      ...
      boTransactionHandler.update(
        transaction.copy(creditCard = Some(creditCard)),
        false)
  ...
  def threeDSCallback(sessionData: SessionData,
    ...
    val parametresProvider =
      parse(org.json4s.StringInput(paymentConfig.cbParam.getOrElse("{}")))
        .extract[Map[String, String]]
    ...
    val ctxMode: String =
      if (Settings.Env == Environment.DEV) "TEST" else "PRODUCTION"
    ...
    try {
      ...
      val requestId: String =
        sessionAndRequestId.substring(sessionAndRequestId.indexOf("|") + 1)
      ...
      val secured: ThreeDSecure =
        SystempayUtilities.create3DSProxy(
          "/wsdl/SystemPay_3DSecure.wsdl",
          new TraceHandler(transaction, "SYSTEMPAY"))

      val paresInfo: PaResInfo = secured.analyzePAResTx(shopId,
                                                        contractNumber,
                                                        ctxMode,
                                                        requestId,
                                                        pares,
      ...
      status match {
        case "N" | "U" =>
          val map = Map("result" -> MogopayConstant.Error,
                        "error.code" -> MogopayConstant.InvalidPassword)
          buildURL(errorURL, map)
        case "Y" | "A" =>
          val resultatPaiement = systempayClient.submit(
            this,
            sessionData,
            sessionData.uuid,
            vendor,
      ...
      case NonFatal(e) =>
        val queryString = Map("result" -> MogopayConstant.Error,
                              "error.code" -> MogopayConstant.UnknownError)
  ...
  override def refund(paymentConfig: PaymentConfig,
    ...
    val newTransactionId = "%06d".format(
      transactionSequenceHandler.nextTransactionId(boTx.vendor.get.uuid))
    ...
    val wsSignature =
      SystempayUtilities.makeSignature(
        certificate,
        parameters.values.toList.asInstanceOf[Seq[String]].asJava)

    val queryOUT = parameters + ("certificate" -> certificate)
    val logOUT = new BOTransactionLog(
      uuid = newUUID,
      provider = "PAYLINE",
      direction = "OUT",
    ...
    val status =
      if (response.getErrorCode == 0) PaymentStatus.REFUNDED
      else PaymentStatus.REFUND_FAILED
    RefundResult(status,
                 response.getErrorCode.toString,
                 SystempayClient.extendedErrorCodes.get(response.getErrorCode))
  }
...
class SystempayClient extends StrictLogging {
  ...
  def submit(paymentHandler: SystempayHandler,
    ...
    if (paymentConfig.paymentMethod == CBPaymentMethod.EXTERNAL) {
      val transDate: String =
        new SimpleDateFormat("yyyyMMddHHmmss").format(paymentRequest.orderDate)
      ...
      val vads_trans_id: String =
        "%06d".format(paymentRequest.transactionSequence.toLong)
      val vads_url_return
        : String = Settings.Mogopay.EndPoint + s"systempay/done/${sessionUUID}"
      ...
      val xmlCalendar: XMLGregorianCalendar =
        DatatypeFactory.newInstance.newXMLGregorianCalendar(gcalendar)
      val expCalendar: GregorianCalendar = new GregorianCalendar
      expCalendar.setTime(paymentRequest.expirationDate)
      val xmlDateExpiration: XMLGregorianCalendar =
        DatatypeFactory.newInstance.newXMLGregorianCalendar(expCalendar)
      ...
      val signature: String = SystempayUtilities.makeSignature(
        ...
        Seq(
          ...
          payment.getCtxMode,
          payment.getComment
        ).asInstanceOf[Seq[String]].asJava
      )

      val wsdlURL: URL =
        getClass.getClassLoader.getResource("wsdl/SystemPay_WSAPI.wsdl")
      val SERVICE_NAME: QName =
        new QName("http://v3.ws.vads.lyra.com/", "StandardWS")
      ...
      paymentResult = paymentResult.copy(
        ...
        transactionDate =
          payment.getTransmissionDate.toGregorianCalendar.getTime,
        transactionCertificate = null,
        transactionSequence = paymentRequest.transactionSequence,
        amount = paymentRequest.amount,
        cardType = paymentRequest.cardType,
        ...
        errorMessageOrigin =
          if (code == 5) Option(info.getExtendedErrorCode)
          else Some(SystempayClient.getExtendedMessage(code)),
        bankErrorCode = info.getAuthResult.toString,
        bankErrorMessage = Some(
          BankErrorCodes.getErrorMessage("%02d".format(info.getAuthResult)))
      )
  ...
  def check3DSecure(sessionData: SessionData,
    ...
    transactionHandler.updateStatus(transactionUUID,
                                    None,
                                    TransactionStatus.VERIFICATION_THREEDS)
    ...
    val cardExpiration =
      new SimpleDateFormat("MMyy").format(paymentRequest.expirationDate)
    val wssignature = SystempayUtilities.makeSignature(
      certificat,
      Seq(shopId,
          contractNumber,
          context,
    ...
    val secured: ThreeDSecure = SystempayUtilities.create3DSProxy(
      "/wsdl/SystemPay_3DSecure.wsdl",
      traceHandler)
    val vresInfo: VeResPAReqInfo = secured.sendVEReqAndbuildPAReqTx(
      shopId,
      contractNumber,
      context,
      cardNumber,
      ...
      cardExpiration,
      wssignature)
    val ctx: util.Map[String, AnyRef] =
      secured.asInstanceOf[BindingProvider].getResponseContext
    val responseHeaders: mutable.Map[String, List[String]] =
      ctx
        .get(MessageContext.HTTP_RESPONSE_HEADERS)
        .asInstanceOf[java.util.Map[String, List[String]]]
        .asScala
    ...
    val headerValues: List[String] = responseHeaders("Set-Cookie")
      .asInstanceOf[java.util.List[String]]
      .asScala
      .toList
    for (headerValue <- headerValues) {
      if (headerValue.startsWith("JSESSIONID")) {
        sessionId =
          headerValue.substring("JSESSIONID=".length, headerValue.indexOf(";"))
      }
    }
    ...
    if ("0" == codeRetour) {
      if ("Y" == enrolled) {
        ...
        result = result.copy(
          ...
          termUrlName = "TermUrl",
          termUrlValue =
            s"${Settings.Mogopay.EndPoint}systempay/3ds-callback/${sessionData.uuid}",
          url = acsURL,
          method = "POST"
        )
    ...
    transactionHandler.updateStatus3DS(transactionUUID,
  ...
  def cancel(paymentHandler: SystempayHandler,
    ...
    transactionHandler.updateStatus(transactionUUID,
                                    None,
                                    TransactionStatus.CANCEL_REQUESTED)
    ...
    paymentResult = paymentResult.copy(
      ...
      errorCodeOrigin = code.toString,
      errorMessageOrigin =
        if (code == 5) Option(response.getExtendedErrorCode)
        else Some(SystempayClient.getExtendedMessage(code)),
      bankErrorCode = "",
      bankErrorMessage = Some("")
    )
...
object SystempayClient {
  val logger =
    LoggerFactory.getLogger("com.mogobiz.pay.handlers.payment.SystempayClient")
  val QUERY_STRING_SEP = "&"
  val QUERY_STRING_ELEMENTS_SEP = "="

  def getExtendedMessage(code: Int): String =
    extendedErrorCodes.getOrElse(code, "")
