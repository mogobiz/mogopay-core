...
case class SubmitParams(successURL: String,
  ...
  def payers: Map[String, Long] =
    _payers
      .map { payers =>
        queryStringToMap(payers, sep = ",", elementsSep = ":")
          .mapValues(_.toLong)
          .map(identity)
      }
      .getOrElse(Map())
...
class TransactionHandler {
  ...
  def retrieveHandler(cbProvider: CBPaymentProvider): CBProvider =
    cbProvider match {
      ...
      case CBPaymentProvider.CUSTOM => {
        if (customPaymentHandler != null) customPaymentHandler
          ...
          throw new InvalidPaymentHandlerException(
            "Custom Payment Handler not found")
      }
      case _ =>
        throw new InvalidPaymentHandlerException(
          "Unable to found Paym Handler for " + cbProvider.toString)
    }

  def validatePayment(
      boShopTransaction: BOShopTransaction): ValidatePaymentResult = {
    retrieveHandler(boShopTransaction.paymentConfig.cbProvider)
      .validatePayment(boShopTransaction)
  }

  def refundPayment(
      boShopTransaction: BOShopTransaction): RefundPaymentResult = {
    retrieveHandler(boShopTransaction.paymentConfig.cbProvider)
      .refundPayment(boShopTransaction)
  }

  def init(sessionData: SessionData,
  ...
  def updateStatus(transactionUUID: String,
    ...
    boTransactionHandler
      .find(transactionUUID)
      .map { transaction =>
        val modStatus = ModificationStatus(
          uuid = newUUID,
          xdate = new Date,
          ipAddr = ipAddress.orElse(
            transaction.modifications
              .collectFirst({ case e => e.ipAddr })
              .flatten),
          ...
          comment = comment
            ...
            .getOrElse(paymentResult
              .map { pr =>
                Option(pr.errorCodeOrigin)
              }
              .getOrElse(None))
        )

        val newPaymentData = paymentResult
          .map { pr =>
        ...
        val transactionDate = paymentResult
          .map { pr =>
        ...
        val newTx = transaction.copy(
          ...
          authorizationId = paymentResult
            .map {
              _.authorizationId
            }
            .getOrElse(transaction.authorizationId),
          errorCodeOrigin = paymentResult
            .map { pr =>
              Option(pr.errorCodeOrigin)
            }
            .getOrElse(transaction.errorCodeOrigin),
          errorMessageOrigin = paymentResult
            .map {
              _.errorMessageOrigin
            }
            .getOrElse(transaction.errorMessageOrigin),
          paymentData = newPaymentData,
          gatewayData = gatewayData
            .map {
              Option(_)
            }
            .getOrElse(transaction.gatewayData),
    ...
    maybeTx
      .map { transaction =>
        ...
        val newTx = transaction.copy(
          ...
          paymentData =
            transaction.paymentData.copy(status3DS = Option(status3DS)),
  ...
  def finishPayment(boTransaction: BOTransaction): BOTransaction = {
    ...
    val boShopTransaction = boShopTransactionHandler
      .findByShopIdAndTransactionUuid(MogopayConstant.SHOP_MOGOBIZ,
                                      boTransaction.transactionUUID)

    // commit du shipping
    val (finalTransaction, finalShopTransaction) =
      if (status == TransactionStatus.PAYMENT_AUTHORIZED) {
        Try(ShippingHandler.confirmShippingPrice(boTransaction.shippingData)) match {
          case Success(shippingData) => {
            val transactionWithShippingData = shippingData
              .map { shippingData =>
                val finalTransWithShippingInfo =
                  boTransaction.copy(shippingData = Some(shippingData))
              ...
              .map { boShopTransaction =>
                ...
                if (validationResult.status == PaymentStatus.COMPLETE) {
                  val updatedTransaction =
                    PaymentHandler.updateTransactionStatus(
                      transactionWithShippingData,
  ...
  def notifyPaymentFinished(transaction: BOTransaction,
                            boShopTransaction: BOShopTransaction): Unit = {
    val locale = transaction.locale
    val localeOrEn = locale.getOrElse("en");
    val vendor = transaction.vendor
    val jsonString =
      BOTransactionJsonTransform.transform(transaction,
                                           boShopTransaction,
                                           LocaleUtils.toLocale(localeOrEn))
    try {
      val (subject, body) =
        templateHandler.mustache(Some(vendor), "mail-order", locale, jsonString)
      EmailHandler.Send(
        Mail(
          vendor.email ->
            s"""${vendor.firstName
    ...
    if (transaction.status == TransactionStatus.COMPLETED) {
      try {
        val (subject, body) = templateHandler.mustache(Some(vendor),
                                                       "mail-bill",
                                                       locale,
                                                       jsonString)
  ...
  def notifySuccessRefund(transaction: BOTransaction,
                          boShopTransaction: BOShopTransaction): Unit = {
      ...
      try {
        val jsonString = BOTransactionJsonTransform
          .transform(transaction,
                     boShopTransaction,
                     LocaleUtils.toLocale(locale.getOrElse("en")))
        val (subject, body) = templateHandler.mustache(Some(vendor),
                                                       "mail-refund",
                                                       locale,
                                                       jsonString)
  ...
  def verify(secret: String,
             amount: Option[Long],
             transactionUUID: String): (BOTransaction, TransactionStatus) = {
    val transaction =
      boTransactionHandler
        .find(transactionUUID)
        .getOrElse(throw TransactionNotFoundException(s"$transactionUUID"))

    if (transaction.vendor.secret != secret)
      throw InvalidMerchantAccountException("")

    val validatedTx =
      if (amount.forall(transaction.amount == _)) transaction
      else throw UnexpectedAmountException(s"$amount")
    ...
    if (!durationOK) {
      ...
      throw TransactionAlreadyConfirmedException(
        MogopayConstant.TransactionAlreadyConfirmed)
    } else {
      val boShopTransaction = boShopTransactionHandler
        .findByShopIdAndTransactionUuid(MogopayConstant.SHOP_MOGOBIZ,
                                        transaction.transactionUUID)
      val expectedSuccessStatus = boShopTransaction
        .map { shop =>
          TransactionStatus.COMPLETED
        }
  ...
  def shippingPrices(
      cart: Cart,
      accountId: String): (Option[AccountAddress], ShippingDataList) = {
    val maybeCustomer = accountHandler.load(accountId)

    val customer =
      maybeCustomer.getOrElse(throw AccountDoesNotExistException(s"$accountId"))

    val address =
      shippingAddressHandler.findByAccount(customer.uuid).find(_.active)

    address
      .map { addr =>
        val internationalUnauthorized = cart.compagnyAddress
          .map { compagnyAddr =>
            !compagnyAddr.shippingInternational && addr.address.country
              .getOrElse("") != compagnyAddr.country
          }
          .getOrElse(false)
        ...
        if (internationalUnauthorized)
          (Some(addr.address),
           ShippingDataList(
             Some(ShippingPriceError.INTERNATIONAL_SHIPPING_NOT_ALLOWED),
             Nil))
  ...
  def selectShippingPrice(sessionData: SessionData,
                          accountId: String,
                          selectShippingDataByShop: Map[String, String])
    : Option[SelectShippingCart] = {
    shippingAddressHandler
      .findByAccount(accountId)
      .find(_.active)
      .map { clientAddress =>
        val internationalUnauthorized = sessionData.cart
          .map { cart =>
            cart.compagnyAddress
              .map { compagnyAddr =>
                !compagnyAddr.shippingInternational && clientAddress.address.country
                  .getOrElse("") != compagnyAddr.country
              }
              .getOrElse(false)
          }
        ...
        if (internationalUnauthorized) None
          ...
          val shippingCart =
            sessionData.shippingCart.getOrElse(throw InvalidContextException(
              "The shippings list wasn't computed."))
          if (shippingCart.hasError)
            throw InvalidContextException(
              "The shippings list must be computed without error.")

          val shippingPriceByShopId =
            shippingCart.shippingPricesByShopId.flatMap {
              shopIdAndShippingPrice =>
                val shopId = shopIdAndShippingPrice._1
                val shippingDataList = shopIdAndShippingPrice._2
                if (shippingDataList.empty) None
                else {
                  val selectShippingDataId = selectShippingDataByShop
                    .get(shopId)
                    .getOrElse(throw NoShippingPriceFound())
                  Some(
                    (shopId -> shippingDataList
                      .findById(selectShippingDataId)
                      .getOrElse(throw NoShippingPriceFound())))
                }
            }
          val selectShippingCart = Some(
            SelectShippingCart(clientAddress.address, shippingPriceByShopId))
  ...
  def submit(submit: Submit): (String, String) = {
    ...
    val vendor =
      if (sessionData.accountId.isDefined && successURL.isEmpty && errorURL.isEmpty && cardinfoURL.isEmpty && authURL.isEmpty) {
        ...
        accountHandler
          .load(sessionData.merchantId.get)
          .getOrElse(throw VendorNotFoundException())
      } else {
        accountHandler
          .load(submit.params.merchantId)
          .getOrElse(throw VendorNotFoundException())
      }
    ...
    var selectedShippingCart =
      sessionData.selectShippingCart.getOrElse(
        throw InvalidContextException("Shipping price cannot be None"))

    val shopCarts = cart.shopCarts.map { shopCart =>
      val shippingCart =
        selectedShippingCart.shippingPriceByShopId.get(shopCart.shopId)
      val shippingPrice = shippingCart
        .map {
          _.price
        }
        .getOrElse(0L)
    ...
    val transaction: Option[BOTransaction] =
    ...
    if (amount.getOrElse(0L) <= 0 && !mogopayAuth)
      throw SomeParameterIsMissingException(
        "Not a mogopay payment and amount is missing.")
    ...
    if (submit.params.customerEmail.isDefined)
    ...
    if (sessionData.mogopay) {
      ...
      if (transactionType.getOrElse("CREDIT_CARD") == "CREDIT_CARD") {
        ...
        if (submit.params.customerCVV.nonEmpty) {
          ...
          val cardNum = SymmetricCrypt.decrypt(card.number,
                                               Settings.Mogopay.CardSecret,
                                               "AES")
          ...
          val paymentRequest = initPaymentRequest(
            vendor.paymentConfig,
            transactionType,
            mogopay = true,
            cartWithShipping,
      ...
      sessionData.paymentRequest = Some(paymentRequest)
      val handler =
        if (submit.sessionData.transactionType.contains("CREDIT_CARD")) {
          val providerName =
            sessionData.paymentConfig.get.cbProvider.toString.toLowerCase
          if (providerName == "custom") {
            val params =
              sessionData.paymentConfig.get.cbParam
  ...
  //
  def download(transactionUuid: String,
               pageFormat: String,
  ...
  def download(transaction: BOTransaction,
    ...
    if (transaction.status == TransactionStatus.COMPLETED) {
      val jsonString =
        BOTransactionJsonTransform.transform(transaction,
                                             boShopTransaction,
                                             LocaleUtils.toLocale(langCountry))
      val (subject, body) =
        templateHandler.mustache(Some(transaction.vendor),
                                 "download-bill",
                                 Some(langCountry),
                                 jsonString)
  ...
  protected def initPaymentRequest(
    ...
    val cc_num =
      if (card_number != null) card_number.replaceAll(" ", "") else null
    val externalPages: Boolean =
      paymentConfig.exists(_.paymentMethod == CBPaymentMethod.EXTERNAL)
    ...
    if (transactionType.getOrElse("CREDIT_CARD") == "CREDIT_CARD" && (!externalPages || mogopay)) {
      if (card_type == null) {
        throw SomeParameterIsMissingException(
          MogopayConstant.CreditCardTypeRequired)
      } else if (cc_num == null) {
        throw SomeParameterIsMissingException(
          MogopayConstant.CreditCardNumRequired)
      } else if (card_month == null) {
        throw SomeParameterIsMissingException(
          MogopayConstant.CreditCardMonthRequired)
      } else if (card_year == null) {
        throw SomeParameterIsMissingException(
          MogopayConstant.CreditCardYearRequired)
      } else if (ccCrypto == null) {
        throw SomeParameterIsMissingException(
          MogopayConstant.CreditCardCryptoRequired)
      } else {
        ...
        if (cc_date.compareTo(new Date()) < 0) {
          throw SomeParameterIsMissingException(
            MogopayConstant.CreditCardExpiryDateInvalid)
        } else {
          paymentRequest.copy(
  ...
  def initGroupPayment(
      token: String): (Account, TransactionRequest, String, String, String) = {
    val decryptedToken =
      SymmetricCrypt.decrypt(token, Settings.Mogopay.Secret, "AES")
    val (expirationDate,
         txUUID,
         customerUUID,
         groupTxUUID,
    ...
    val txReq = transactionRequestHandler
      .find(txUUID)
      .getOrElse(throw new TransactionNotFoundException(txUUID))
...
object BOTransactionJsonTransform {
  ...
  def transformAsJValue(transaction: BOTransaction,
                        shopTransaction: BOShopTransaction,
                        locale: Locale) = {
  ...
  def transform(transaction: BOTransaction,
                shopTransaction: BOShopTransaction,
                locale: Locale) = {
    JacksonConverter.asString(
      transformAsJValue(transaction, shopTransaction, locale))
  }
  ...
  private def transformBOTransaction(obj: JValue, locale: Locale): JValue = {
    obj match {
      case t: JObject => {
        val currencyCode =
          getFieldAsString(t \ "currency" \ "code").getOrElse("")
        val fractionDigits = getFieldAsBigInt(t \ "currency" \ "fractionDigits")
          .getOrElse(BigInt(2))
  ...
  private def transformShopBOTransaction(obj: JValue,
                                         locale: Locale): JValue = {
    obj match {
      case t: JObject => {
        val currencyCode =
          getFieldAsString(t \ "currency" \ "code").getOrElse("")
        ...
        val transformChildren = filterChildren.map { child: JField =>
          child match {
            case JField("status", status: JValue) =>
              JField("status",
                     JString(getFieldAsString(status \ "name").getOrElse("")))
            case JField("amount", JInt(amount)) =>
              JField("amount",
                     formatPrice(locale, amount, currencyCode, fractionDigits))
            case JField("endDate", JString(endDate)) =>
              JField("endDate", getDateAsMillis(endDate))
            ...
            case JField("extra", JString(extra)) =>
              JField(
                "cart",
                transformShopCart(parse(extra).extract[ShopCartWithShipping],
                                  locale))
            case JField("creditCard", creditCard: JValue) =>
              JField("creditCard", transformBOCreditCard(creditCard))
            case JField("vendor", vendor: JValue) =>
              JField("vendor", transformAccount(vendor))
            case JField("customer", customer: JValue) =>
              JField("customer", transformAccount(customer))
            case f: JField => f
          }
        }
        JObject(transformChildren)
        ...
        val transformChildren = t.obj.map { child: JField =>
          child match {
            case JField("paymentType", paymentType: JValue) =>
              JField(
                "paymentType",
                JString(getFieldAsString(paymentType \ "name").getOrElse("")))
            case JField("cbProvider", cbProvider: JValue) =>
              JField(
                "cbProvider",
                JString(getFieldAsString(cbProvider \ "name").getOrElse("")))
            case JField("orderDate", JString(orderDate)) =>
              JField("orderDate", getDateAsMillis(orderDate))
            case JField("status3DS", status3DS: JValue) =>
              JField(
                "status3DS",
                JString(getFieldAsString(status3DS \ "name").getOrElse("")))
            case f: JField => f
          }
        }
        JObject(transformChildren)
        ...
        val transformChildren = t.obj.map { child: JField =>
          child match {
            case JField("cardType", cardType: JValue) =>
              JField("cardType",
                     JString(getFieldAsString(cardType \ "name").getOrElse("")))
            case JField("expiryDate", JString(expiryDate)) =>
              JField("expiryDate", getDateAsMillis(expiryDate))
            case f: JField => f
          }
        }
        JObject(transformChildren)
        ...
        val transformChildren = filterChildren.map { child: JField =>
          child match {
            case JField("civility", civility: JValue) =>
              JField("civility",
                     JString(getFieldAsString(civility \ "name").getOrElse("")))
            case JField("status", status: JValue) =>
              JField("status",
                     JString(getFieldAsString(status \ "name").getOrElse("")))
            case JField("address", address: JValue) =>
              JField("address", transformAccountAddress(address))
            case JField("birthDate", JString(birthDate)) =>
              JField("birthDate", getDateAsMillis(birthDate))
            case JField("shippingAddresses", shippingAddresses: JValue) =>
              JField("shippingAddress",
                     transformShippingAddress(shippingAddresses))
            case f: JField => f
          }
        }
        JObject(transformChildren)
        ...
        val transformChildren = filterChildren.map { child: JField =>
          child match {
            ...
            case JField("status", status: JValue) =>
              JField("status",
                     JString(getFieldAsString(status \ "name").getOrElse("")))
            case JField("telephone", telephone: JValue) =>
              JField("telephone", transformTelephone(telephone))
            case f: JField => f
          }
        }
        JObject(transformChildren)
  ...
  private def transformShopCart(shopCart: ShopCartWithShipping,
                                locale: Locale): JValue = {
    ...
    JObject(
      JField("shipping",
             formatPrice(locale,
                         shopCart.shippingPrice,
                         currencyCode,
                         fractionDigits)),
      JField("price",
             formatPrice(locale, shopCart.price, currencyCode, fractionDigits)),
      JField(
        "taxAmount",
        formatPrice(locale, shopCart.taxAmount, currencyCode, fractionDigits)),
      JField(
        "endPrice",
        formatPrice(locale, shopCart.endPrice, currencyCode, fractionDigits)),
      JField(
        "reduction",
        formatPrice(locale, shopCart.reduction, currencyCode, fractionDigits)),
      JField(
        "finalPrice",
        formatPrice(locale, shopCart.finalPrice, currencyCode, fractionDigits)),
  ...
  private def transformCartItem(cartItem: CartItem,
    ...
    JObject(
      ...
      JField("price",
             formatPrice(locale, cartItem.price, currencyCode, fractionDigits)),
      JField(
        "endPrice",
        formatPrice(locale, cartItem.endPrice, currencyCode, fractionDigits)),
      JField("tax", JDouble(cartItem.tax)),
      JField(
        "taxAmount",
        formatPrice(locale, cartItem.taxAmount, currencyCode, fractionDigits)),
      JField(
        "totalPrice",
        formatPrice(locale, cartItem.totalPrice, currencyCode, fractionDigits)),
      JField("totalEndPrice",
             formatPrice(locale,
                         cartItem.totalEndPrice,
                         currencyCode,
                         fractionDigits)),
      JField("totalTaxAmount",
             formatPrice(locale,
                         cartItem.totalTaxAmount,
                         currencyCode,
                         fractionDigits)),
      JField(
        "salePrice",
        formatPrice(locale, cartItem.salePrice, currencyCode, fractionDigits)),
      JField("saleEndPrice",
             formatPrice(locale,
                         cartItem.saleEndPrice,
                         currencyCode,
                         fractionDigits)),
      JField("saleTaxAmount",
             formatPrice(locale,
                         cartItem.saleTaxAmount,
                         currencyCode,
                         fractionDigits)),
      JField("saleTotalPrice",
             formatPrice(locale,
                         cartItem.saleTotalPrice,
                         currencyCode,
                         fractionDigits)),
      JField("saleTotalEndPrice",
             formatPrice(locale,
                         cartItem.saleTotalEndPrice,
                         currencyCode,
                         fractionDigits)),
      JField("saleTotalTaxAmount",
             formatPrice(locale,
                         cartItem.saleTotalTaxAmount,
                         currencyCode,
                         fractionDigits)),
      JField(
        "registeredCartItems",
        JArray(cartItem.registeredCartItems.toList.map { registeredCartItem =>
          transformRegisteredCartItem(registeredCartItem,
                                      locale,
                                      currencyCode,
                                      fractionDigits)
        })
      ...
      JField("downloadableLink",
             if (cartItem.downloadableLink != null)
               JString(cartItem.downloadableLink)
             else JNothing)
    ).merge(Extraction.decompose(cartItem.customs))
  }

  private def transformRegisteredCartItem(
  ...
  private def transformCoupon(coupon: Coupon,
                              locale: Locale,
                              currencyCode: String,
                              fractionDigits: Int): JValue = {
    JObject(
      ...
      JField("endDate",
             coupon.endDate
               ...
               .getOrElse(JNothing)),
      JField("price",
             formatPrice(locale, coupon.price, currencyCode, fractionDigits)),
      JField("customs", Extraction.decompose(coupon.customs))
    )
  }

  private def transformExtra(
      locale: Locale,
      currencyCode: String,
      fractionDigits: BigInt): PartialFunction[JField, JField] = {
    case JField("salePrice", JInt(salePrice)) =>
      JField("salePrice",
             formatPrice(locale, salePrice, currencyCode, fractionDigits))
    case JField("saleEndPrice", JInt(saleEndPrice)) =>
      JField("saleEndPrice",
             formatPrice(locale, saleEndPrice, currencyCode, fractionDigits))
    case JField("totalPrice", JInt(totalPrice)) =>
      JField("totalPrice",
             formatPrice(locale, totalPrice, currencyCode, fractionDigits))
    case JField("totalEndPrice", JInt(totalEndPrice)) =>
      JField("totalEndPrice",
             formatPrice(locale, totalEndPrice, currencyCode, fractionDigits))
    case JField("saleTotalPrice", JInt(saleTotalPrice)) =>
      JField("saleTotalPrice",
             formatPrice(locale, saleTotalPrice, currencyCode, fractionDigits))
    case JField("saleTotalEndPrice", JInt(saleTotalEndPrice)) =>
      JField(
        "saleTotalEndPrice",
        formatPrice(locale, saleTotalEndPrice, currencyCode, fractionDigits))
    case JField("startDate", JString(startDate)) =>
      JField("startDate", getDateAsMillis(startDate))
    case JField("endDate", JString(endDate)) =>
      JField("endDate", getDateAsMillis(endDate))
    case JField("birthdate", JString(birthdate)) =>
      JField("birthdate", getDateAsMillis(birthdate))
  ...
  private def formatPrice(locale: Locale,
                          amount: BigInt,
                          currencyCode: String,
                          fractionDigits: BigInt) = {
