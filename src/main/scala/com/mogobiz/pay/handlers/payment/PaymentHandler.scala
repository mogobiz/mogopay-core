...
case class ExternalPaymentResult(paymentUuid: String,
                                 redirection: Either[Uri, FormRedirection])

case class AuthorizedPaymentResult(
    paymentUuid: String,
    @JsonScalaEnumeration(classOf[PaymentStatusRef]) status: PaymentStatus.PaymentStatus)

case class FormRedirection(html: String)

case class ValidatePaymentResult(
    @JsonScalaEnumeration(classOf[PaymentStatusRef]) status: PaymentStatus.PaymentStatus,
    transactionId: Option[String],
    transactionDate: Option[Date],
    boShopTransaction: BOShopTransaction)

case class RefundPaymentResult(
    @JsonScalaEnumeration(classOf[PaymentStatusRef]) status: PaymentStatus.PaymentStatus,
    transactionId: Option[String],
    transactionDate: Option[Date],
    boShopTransaction: BOShopTransaction)
...
trait PaymentHandler extends StrictLogging {
  ...
  def createBOTransaction(
      sessionData: SessionData,
      paymentRequest: PaymentRequest,
      paymentType: PaymentType.PaymentType): BOTransaction = {
    val vendorId =
      sessionData.merchantId.getOrElse(
        throw new InvalidContextException(
          "Merchant UUID not found in session data"))
    val vendor = accountHandler
      .load(vendorId)
      .getOrElse(throw new InvalidContextException("Vendor not found"))
    val customer = sessionData.accountId.flatMap { accountHandler.load(_) }
    val callbackUrl =
      sessionData.successURL.getOrElse(
        throw InvalidContextException("callbackUrl not found in session data"))
    val shippingData =
      sessionData.selectShippingCart.flatMap(
        _.shippingPriceByShopId.get(MogopayConstant.SHOP_MOGOBIZ))
    val transactionUUID = sessionData.transactionUuid.getOrElse(
      throw InvalidContextException(
        "Transaction UUID not found in session data"))
    val paymentConfig: PaymentConfig =
      sessionData.paymentConfig.getOrElse(
        throw InvalidContextException(
          "Payment Config not found in session data"))
    val email = sessionData.email.getOrElse(
      customer
        .map { _.email }
        .getOrElse(
          throw InvalidContextException("Email not found in session data")))
  ...
  def finishTransaction(boTransaction: BOTransaction,
                        status: TransactionStatus.TransactionStatus,
                        error: Option[String] = None): BOTransaction = {
    val newBOTransaction =
      PaymentHandler.updateTransactionStatus(
        boTransaction.copy(endDate = Some(new Date)),
        status,
        error)
  ...
  def updateBOShopTransactionStatus(
      boShopTransaction: BOShopTransaction,
      status: ShopTransactionStatus.ShopTransactionStatus,
      errorCode: Option[String]): BOShopTransaction = {
    updateBOShopTransactionStatusAndData(boShopTransaction,
                                         status,
                                         errorCode,
                                         boShopTransaction.paymentData)
  }

  def updateBOShopTransactionStatusAndData(
      boShopTransaction: BOShopTransaction,
      status: ShopTransactionStatus.ShopTransactionStatus,
    ...
    val newBOShopTransaction = boShopTransaction.copy(
      status = status,
      errorCode = errorCode,
      paymentData = paymentData,
  ...
  protected def createTransactionLog(
      boShopTransaction: BOShopTransaction,
      direction: String,
      transactionStep: TransactionShopStep.TransactionShopStep,
      data: List[String]): Unit = {
    val botlog = BOTransactionLog(
      newUUID,
      direction,
      data.mkString("<br/>"),
  ...
  protected def getCreditCardConfig(
      paymentConfig: PaymentConfig): Map[String, String] = {
    import com.mogobiz.pay.implicits.Implicits._
    paymentConfig.cbParam
  ...
  protected def redirectionToCallback(boTransaction: BOTransaction): Uri = {
    ...
    Uri(
      boTransaction.callbackUrl + sep + GlobalUtil.mapToQueryString(
        query.toMap))
  }
  /*
...
object PaymentHandler {
  ...
  def updateTransactionStatus(
      ...
      msgError: Option[String] = None): BOTransaction = {
    val newBOTransaction = boTransaction
      .copy(status = status,
