...
case class ValidatePaymentResult(@JsonScalaEnumeration(classOf[PaymentStatusRef]) status: PaymentStatus.PaymentStatus,
                                 transactionId: Option[String],
                                 transactionDate: Option[Date],
                                 boShopTransaction: BOShopTransaction)

case class RefundPaymentResult(@JsonScalaEnumeration(classOf[PaymentStatusRef]) status: PaymentStatus.PaymentStatus,
                               transactionId: Option[String],
                               transactionDate: Option[Date],
                               boShopTransaction: BOShopTransaction)
...
trait PaymentHandler extends StrictLogging {
  ...
  def createBOTransaction(sessionData: SessionData,
    ...
    val callbackUrl =
      sessionData.successURL.getOrElse(throw InvalidContextException("callbackUrl not found in session data"))
    val shippingData =
      sessionData.selectShippingCart.flatMap(_.shippingPriceByShopId.get(MogopayConstant.SHOP_MOGOBIZ))
    val transactionUUID = sessionData.transactionUuid.getOrElse(
        throw InvalidContextException("Transaction UUID not found in session data"))
    val paymentConfig: PaymentConfig =
      sessionData.paymentConfig.getOrElse(throw InvalidContextException("Payment Config not found in session data"))
    val email = sessionData.email.getOrElse(
        customer.map { _.email }.getOrElse(throw InvalidContextException("Email not found in session data")))
  ...
  def finishTransaction(boTransaction: BOTransaction,
    ...
    val newBOTransaction =
      PaymentHandler.updateTransactionStatus(boTransaction.copy(endDate = Some(new Date)), status, error)
  ...
  protected def createTransactionLog(boShopTransaction: BOShopTransaction,
                                     direction: String,
...
object PaymentHandler {
  ...
  def updateTransactionStatus(boTransaction: BOTransaction,
                              status: TransactionStatus.TransactionStatus,
                              error: Option[String] = None,
                              msgError: Option[String] = None): BOTransaction = {
    val newBOTransaction = boTransaction
      .copy(status = status, transactionDate = Calendar.getInstance().getTime, error = error, msgError = msgError)
