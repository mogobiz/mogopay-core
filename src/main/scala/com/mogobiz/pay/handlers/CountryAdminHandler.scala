...
class CountryAdminHandler {
  def admins1(countryCode: String): Seq[CountryAdmin] = {
    val req = search in Settings.Mogopay.EsIndex -> "CountryAdmin" filter and(
      termFilter("level" -> 1),
  ...
  def cities(countryCode: Option[String] = None,
    ...
    val req = search in Settings.Mogopay.EsIndex -> "CountryAdmin" filter and(filters.flatten: _*) size Integer.MAX_VALUE
    EsClient.searchAll[CountryAdmin](req) sortBy (_.name)
  }
}
