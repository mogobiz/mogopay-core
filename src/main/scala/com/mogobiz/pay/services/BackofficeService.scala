...
class BackofficeService extends Directives with DefaultComplete {
  ...
  lazy val listCustomers = path("customers") {
    get {
      parameters('page.as[Int], 'max.as[Int]) { (page, max) =>
        session { session =>
          handleCall(
            backofficeHandler.listCustomers(session.sessionData, page, max),
  ...
  lazy val listTransactionLogs = path("transactions" / Segment / "logs") {
    transactionId =>
      get {
        session { session =>
          handleCall(
      ...
      session { session =>
        ...
        params {
          (email,
           ...
           deliveryStatus) =>
            handleCall(
              backofficeHandler.listTransactions(
                session.sessionData,
  ...
  lazy val getTransaction = path("transaction" / JavaUUID) { uuid =>
    get {
      session { session =>
        handleCall(
          backofficeHandler.getTransaction(uuid.toString),
          (trans: Option[BOTransaction]) => complete(StatusCodes.OK -> trans))
      }
    }
  }

  lazy val listShopTransaction =
    path("transactions" / Segment / "shopTransactions") { transactionId =>
      get {
        session { session =>
          handleCall(
            backofficeHandler.listShopTransactions(transactionId),
