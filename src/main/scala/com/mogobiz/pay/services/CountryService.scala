...
class CountryService extends Directives with DefaultComplete {
  ...
  lazy val countriesForShipping = path("countries-for-shipping") {
    get {
      handleCall(
        countryHandler.findCountriesForShipping(),
  ...
  lazy val countriesForBilling = get {
    path("countries-for-billing") {
      handleCall(
        countryHandler.findCountriesForBilling(),
        (countries: Seq[Country]) => complete(StatusCodes.OK -> countries))
    }
  }
  ...
  lazy val countryPath =
    get {
      path("country" / Segment / "admin1" / Segment) {
        (countryCode, admin1Code) =>
          handleCall(countryAdminHandler.getAdmin1ByCode(countryCode,
                                                         admin1Code),
                     (country: Option[CountryAdmin]) =>
                       complete(StatusCodes.OK -> country))
      }
    } ~
      get {
        path("country" / Segment / "admin2" / Segment) {
          (countryCode, admin2Code) =>
            handleCall(countryAdminHandler.getAdmin2ByCode(countryCode,
                                                           admin2Code),
                       (country: Option[CountryAdmin]) =>
                         complete(StatusCodes.OK -> country))
        }
      ...
      get {
        path("country" / Segment) { countryCode =>
          handleCall(
            countryHandler.findByCode(countryCode),
  ...
  lazy val admins1 = path("admins1" / Segment) { countryCode =>
    get {
      handleCall(
        countryAdminHandler.admins1(countryCode),
  ...
  lazy val cities = path("cities") {
    get {
      val params = parameters('country.?,
                              'parent_admin1_code.?,
                              'parent_admin2_code.?,
                              'name.?)
      params { (c, a1, a2, name) =>
        handleCall(
          countryAdminHandler.cities(c, a1, a2, name),
          (admins: Seq[CountryAdmin]) => complete(StatusCodes.OK -> admins))
      }
    }
  }
  ...
  lazy val admins2 = path("admins2" / Segment) { admin1 =>
    get {
      handleCall(
        countryAdminHandler.admins2(admin1),
        (admins: Seq[CountryAdmin]) => complete(StatusCodes.OK -> admins))
    }
  }
    ...
    (countryCode, phone) =>
      get {
        handleCall(countryHandler.checkPhoneNumber(phone, countryCode),
                   (phoneVerif: PhoneVerification) =>
                     complete(StatusCodes.OK -> phoneVerif))
      }
  }
}
