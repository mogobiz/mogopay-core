...
import com.mogobiz.pay.model.Mogopay.{Account, TransactionRequest}
...
class TransactionService(implicit executionContext: ExecutionContext)
  ...
  lazy val init = path("init") {
    post {
      formFields(
        'merchant_secret,
        'transaction_amount.?.as[Option[Long]],
        'return_url.?,
        'group_payment_exp_date.?.as[Option[Long]],
        'group_payment_refund_percentage.?.as[Option[Int]]
      ).as(TransactionInit) { params =>
        session { session =>
          {
            import Implicits._
            ...
            handleCall(transactionHandler.init(session.sessionData,
                                               params,
                                               cart),
                       (id: String) =>
  ...
  lazy val verify = path("verify") {
    ...
    get {
      val params = parameters('merchant_secret,
                              'transaction_amount.?.as[Option[Long]],
                              'transaction_id)
      params { (secret, amount, transactionUUID) =>
        handleCall(
          ...
          (result: (BOTransaction, TransactionStatus.TransactionStatus)) => {
            ...
            complete(
              StatusCodes.OK -> Map(
                'result -> (if (transaction.status == expectedSuccessStatus)
                              "success"
                            else "error"),
                'transaction_id -> URLEncoder
                  .encode(transaction.transactionUUID, "UTF-8"),
                'transaction_amount -> URLEncoder
                  .encode(transaction.amount.toString, "UTF-8"),
                'transaction_email -> Option(transaction.email).getOrElse(""),
                'transaction_status -> URLEncoder
                  .encode(transaction.status.toString, "UTF-8"),
                ...
                'transaction_end -> URLEncoder.encode(
                  new SimpleDateFormat("yyyyMMddHHmmss")
                    .format(transaction.dateCreated),
                  "UTF-8"),
                'transaction_providerid -> URLEncoder.encode(transaction.uuid,
                                                             "UTF-8"),
                'transaction_type -> URLEncoder
                  .encode(transaction.paymentType.toString, "UTF-8"),
  ...
  lazy val initGroupPayment = path("init-group-payment") {
    get {
      val params = parameters('token, 'transaction_type, 'card_cvv, 'card_month,
        'card_year, 'card_type, 'card_number)
      params {
        (token, transactionType, ccCVV, ccMonth, ccYear, ccType, ccNumber) =>
          session { session =>
            handleCall(
              transactionHandler.initGroupPayment(token),
  ...
  lazy val submit = path("submit") {
    post {
      formFields(
        ...
        'group_tx_uuid.?,
        'locale.?
      ).as(SubmitParams) { submitParams: SubmitParams =>
        ...
        if (!submitParams.payers.isEmpty && payersAmountsSum != submitParams.amount) {
          ...
          session { session =>
            ...
            if (submitParams.payers.nonEmpty && !submitParams.payers.keys.toList
                  .contains(session.sessionData.email.get)) {
              complete(
                StatusCodes.BadRequest -> "The payers' list doesn't contain the current user.")
            } else {
              //clientIP { ip =>
              session.sessionData.ipAddress = Some("192.168.1.1") //Some(ip.toString)
              session.sessionData.payers =
                submitParams.payers.toMap[String, Long]
        ...
        session { session =>
          ...
          session.sessionData.accountId match {
            case Some(accountId: String) =>
              handleCall(transactionHandler.download(transactionUuid,
                                                     pageFormat,
                                                     langCountry),
                         (pdfFile: File) => {
                           getFromFile(pdfFile)
            ...
            case _ =>
              completeException(new UnauthorizedException("Not logged in"))
          }
        }
      }
    }
  ...
  protected def doSubmit(submitParams: SubmitParams,
    ...
    def isNewSession: Boolean = {
      val sessionTrans =
        session.sessionData.transactionUuid.getOrElse("__SESSION_UNDEFINED__")
      val incomingTrans = submitParams.transactionUUID
      sessionTrans != incomingTrans
    }

    if (!Settings.Mogopay.Anonymous && submitParams.merchantId != session.sessionData.merchantId
          .getOrElse("__MERCHANT_UNDEFINED__")) {
      ...
      handleCall(
        transactionHandler.submit(
          Submit(session.sessionData, submitParams, None, None)),
        (t: (String, String)) => {
          val (serviceName, methodName) = t
          setSession(session) {

            val sessionId = session.id
            val url = s"${Settings.Mogopay.EndPoint}$serviceName/$methodName/$sessionId")
              ...
              response: HttpResponse =>
                response.status match {
                  ...
                  case _ =>
                    ...
                    setSession(session) {
                      def toHTTPResponse(t: Throwable): StatusCode =
                        exception match {
