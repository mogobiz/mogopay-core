...
class PaylineService
  ...
  lazy val startPayment = path("start" / Segment) { xtoken =>
    ...
    get {
      parameterMap { params =>
        ...
        handleCall(
          paylineHandler.startPayment(session.sessionData),
          (data: Either[FormRedirection, Uri]) =>
            setSession(session) {
              data match {
                case Left(content) =>
                  complete(
  ...
  lazy val done = path("done" / Segment / Segment) {
    (xtoken, boShopTransaction) =>
      get {
        parameterMap { params =>
          import Implicits._
          val session = SessionESDirectives.load(xtoken)
          handleCall(
            paylineHandler.done(boShopTransaction, params),
            (data: Uri) =>
              session
                .map { session =>
                  setSession(session) {
    ...
    (xtoken, boShopTransaction) =>
      get {
        parameterMap { params =>
          ...
          val session = SessionESDirectives.load(xtoken)
          handleCall(
            paylineHandler.callbackPayment(boShopTransaction, params),
            (pr: Unit) =>
              session
                .map { session =>
                  setSession(session) {
  ...
  lazy val threeDSCallback = path("3ds-callback" / Segment / Segment) {
    (xtoken, boTransactionUuid) =>
      post {
        entity(as[FormData]) { formData =>
          ...
          handleCall(
            paylineHandler.threeDSCallback(sessionData,
                                           boTransactionUuid,
