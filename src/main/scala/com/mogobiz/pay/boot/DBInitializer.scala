...
object DBInitializer {
  ...
  private def fillDB() {
    val PAYPAL = Map(
      "paypalUser" -> "hayssams-facilitator_api1.yahoo.com",
      "paypalPassword" -> "1365940711",
      "paypalSignature" -> "An5ns1Kso7MWUdW4ErQKJJJ4qi4-AIvKXMZ8RRQl6BBiVO5ISM9ECdEG"
    )
    val PAYLINE =
      Map("paylineAccount" -> "26399702760590",
          "paylineKey" -> "SH0gPsNhvHmePmlZz3Mj",
          "paylineContract" -> "1234567")
    val SIPS =
      Map("sipsMerchantId" -> "011223344553333", "sipsMerchantCountry" -> "fr")
    val SIPS_2 =
      Map("sipsMerchantId" -> "011223344551112", "sipsMerchantCountry" -> "fr")
    ...
    val AUTHORIZENET = Map("apiLoginID" -> "5zLq4S76A",
                           "transactionKey" -> "4tMKn44B9a8Cn9C5",
                           "md5Key" -> "wilson")
    ...
    val paypalPaylineExternal = createPaymentConfig(CBPaymentProvider.PAYLINE,
                                                    ...
                                                    Some(1),
                                                    Some("""\d+"""))
    val merchantAccount = createMerchantAccount(
      "ebc23bd9-3abc-4684-849d-e4e15c1a0f82",
    ...
    val paypalSips2DSConfig =
      createPaymentConfig(CBPaymentProvider.SIPS,
                          PAYPAL,
                          Map(),
                          SIPS,
                          CBPaymentMethod.THREEDS_NO)
    val merchantAccount2 = createMerchantAccount(
      "c3a4548f-1edf-4a1f-8a9a-4f0b374720cd",
    ...
    val paypalPayboxExternalConfig =
      createPaymentConfig(CBPaymentProvider.PAYBOX,
                          PAYPAL,
                          Map(),
                          PAYBOX_EXTERNAL,
                          CBPaymentMethod.EXTERNAL)
    val merchantAccount3 = createMerchantAccount(
      "d389ae7-5136-42ef-a6d6-f2f02cee075d",
    ...
    val paypalPaybox2DSConfig =
      createPaymentConfig(CBPaymentProvider.PAYBOX,
                          ...
                          PAYBOX_2DS,
                          CBPaymentMethod.THREEDS_NO)
    val merchantAccount4 = createMerchantAccount(
      "17727558-970e-40f3-9fa1-89319995891c",
    ...
    val paypalPayline2DSConfig =
      createPaymentConfig(CBPaymentProvider.PAYLINE,
                          PAYPAL,
                          Map(),
                          PAYLINE,
                          CBPaymentMethod.THREEDS_NO)
    val merchantAccount5 = createMerchantAccount(
      "5873f47c-afb4-4cb7-bc76-16ca88c389e7",
    ...
    val paypalPayline3DSConfig =
      createPaymentConfig(CBPaymentProvider.PAYLINE,
                          ...
                          Map(),
                          PAYLINE,
                          CBPaymentMethod.THREEDS_REQUIRED)
    val merchantAccount6 = createMerchantAccount(
      "30958ef7-fad3-4f29-988e-df51376974cd",
    ...
    val paypalSystemPayExternalConfig =
      createPaymentConfig(CBPaymentProvider.SYSTEMPAY,
                          PAYPAL,
                          Map(),
                          SYS_PAY,
                          CBPaymentMethod.EXTERNAL)
    val merchantAccount7 = createMerchantAccount(
      "d7b864c8-4567-4603-abd4-5f85e9ff56e6",
    ...
    val paypalSystemPay2DSConfig =
      createPaymentConfig(CBPaymentProvider.SYSTEMPAY,
                          ...
                          SYS_PAY,
                          CBPaymentMethod.THREEDS_NO)
    val merchantAccount8 = createMerchantAccount(
      "78a2fa03-5498-4f07-b716-c9b9c2b64954",
    ...
    val paypalSystemPay3DSConfig =
      createPaymentConfig(CBPaymentProvider.SYSTEMPAY,
                          ...
                          SYS_PAY,
                          CBPaymentMethod.THREEDS_REQUIRED)
    val merchantAccount9 = createMerchantAccount(
      "92795318-8760-4a5f-b71a-c7dcf4af2b79",
    ...
    val paypalSipsExternalConfig =
      createPaymentConfig(CBPaymentProvider.SIPS,
                          ...
                          SIPS_2,
                          CBPaymentMethod.EXTERNAL)
    val merchantAccount10 = createMerchantAccount(
      "f56269e3-7d22-4dcf-9812-ff74a7d7d2c7",
    ...
    val paypalPaybox3DSConfig =
      createPaymentConfig(CBPaymentProvider.PAYBOX,
                          ...
                          PAYBOX_3DS,
                          CBPaymentMethod.THREEDS_REQUIRED)
    val merchantAccount11 = createMerchantAccount(
      "e7542826-f6cc-46bd-8f61-550b7fea9ca7",
    ...
    val paypalSips3DSPaymentConfig =
      createPaymentConfig(CBPaymentProvider.SIPS,
                          ...
                          SIPS,
                          CBPaymentMethod.THREEDS_REQUIRED)
    val merchantAccount12 = createMerchantAccount(
      "7264a70e-9960-4492-b466-4377a1fc2025",
    ...
    val systemPayCustomConfig = createPaymentConfig(
      cbProvider = CBPaymentProvider.SYSTEMPAY,
      paypalConfig = PAYPAL,
      applePayConfig = Map(),
      cbConfig = SYS_PAY,
    ...
    val merchantAccountInfoSystempayCustom = createMerchantAccount(
      "cccccccc-4567-4602-abd5-4f85e9ff56e6",
      "mogopay-systempay-custom@merchant.com",
      "Mogopay Systempay Custom",
      "TEST",
      systemPayCustomConfig)

    val payboxCustomConfig = createPaymentConfig(
      cbProvider = CBPaymentProvider.PAYBOX,
      paypalConfig = PAYPAL,
      applePayConfig = Map(),
      cbConfig = PAYBOX_2DS,
      cbMethod = CBPaymentMethod.THREEDS_NO,
    ...
    val merchantAccountInfoPayboxCustom = createMerchantAccount(
      "dddddddd-4567-4602-abd5-4f85e9ff56e6",
      "mogopay-paybox-custom@merchant.com",
      "Mogopay Paybox Custom",
      "TEST",
      payboxCustomConfig)

    val paylineCustomConfig = createPaymentConfig(
      cbProvider = CBPaymentProvider.PAYLINE,
      paypalConfig = PAYPAL,
      applePayConfig = Map(),
      cbConfig = PAYLINE,
      cbMethod = CBPaymentMethod.THREEDS_NO,
    ...
    val merchantAccountInfoPaylineCustom = createMerchantAccount(
      "eeeeeeee-4567-4602-abd5-4f85e9ff56e6",
      "mogopay-payline-custom@merchant.com",
      "Mogopay Payline Custom",
      "TEST",
      paylineCustomConfig)

    val sipsCustomConfig = createPaymentConfig(
      cbProvider = CBPaymentProvider.SIPS,
      paypalConfig = PAYPAL,
      applePayConfig = Map(),
      cbConfig = SIPS,
      cbMethod = CBPaymentMethod.THREEDS_NO,
    ...
    val merchantAccountInfoSIPSCustom = createMerchantAccount(
      "ffffffff-4567-4602-abd5-4f85e9ff56e6",
    ...
    val paymentConfig12External = createPaymentConfig(
    ...
    val paymentConfig12Custom = createPaymentConfig(
      CBPaymentProvider.AUTHORIZENET,
      PAYPAL,
      Map(),
      AUTHORIZENET,
    ...
    var merchantAccountInfoANetExternal = createMerchantAccount(
      "f802a048-e8ec-4619-abf0-d3a0e0eecc2e",
      "mogopay-anet-external@merchant.com",
      "ANET External",
      "Merchant",
      paymentConfig12External)
    var merchantAccountInfoANetCustom = createMerchantAccount(
      "f5c4a907-6f73-4ecf-ba34-3c7d97d3d6ba",
    ...
    val client1Account = createClientAccount(
      "8a53ef3e-34e8-4569-8f68-ac0dfc548a0f",
      "client@merchant.com",
      "Client 1",
      "TEST",
      merchantAccount7,
    ...
    createClientAccount(
      "15995735-56ca-4d19-806b-a6bc7fedc162",
    ...
    createClientAccount("d31bbce4-29b4-465a-aedd-1784ee6e3929",
                        ...
                        false)
    createClientAccount(
      "cccccccc-29b4-465a-aedd-1784ee6e3929",
    ...
    val applePayConfig = createPaymentConfig(CBPaymentProvider.NONE,
                                             ...
                                             Some(42),
                                             Some("""\d+"""))
    var applePayMerchant = createMerchantAccount(
      "aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa",
      "mogopay-apay@merchant.com",
      "Merchant",
      "Mogopay",
      applePayConfig)
    val applePayClient = createClientAccount(
  ...
  private def createAddress(
    ...
    val phone = Telephone("+33123456789",
                          "0123456789",
                          "FR",
                          Some("000"),
                          telephoneStatus)
  ...
  private def createShippingAddress(firstname: String,
                                    lastname: String,
                                    active: Boolean): ShippingAddress = {
    val prefixe = if (active) "Active " else ""
    ShippingAddress(uuid = UUID.randomUUID().toString,
                    active = active,
                    address =
                      createAddress(prefixe + firstname, prefixe + lastname))
  }
  ...
  private def createTransaction(uuid: String,
    ...
    val currency =
      CartRate("EUR", Currency.getInstance("EUR").getNumericCode, 0.01, 2)
    val creditCard =
      BOCreditCard("1234XXXXXXXXXXX9087", None, new Date(), CreditCardType.CB)
    val paymentData = BOPaymentData(
    ...
    val shopTransaction = BOShopTransaction(
...
+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++END
    ...
    Files.write(certifTargetFile.toPath,
...
%>
    ...
    Files.write(parcomTargetFile.toPath,
                parcomContent.getBytes(StandardCharsets.UTF_8))

    val parcomDefaultTargetFile = new File(certifDir, "parcom.default")
    parcomDefaultTargetFile.delete()
    Files.write(parcomDefaultTargetFile.toPath,
                parcomDefaultContent.getBytes(StandardCharsets.UTF_8))

    var certifTargetFile = new File(certifDir, "certif.fr.011223344551112.jsp")
    certifTargetFile.delete()
    Files.write(certifTargetFile.toPath,
                certifContent.getBytes(StandardCharsets.UTF_8))
...
object DbInitMain extends App {
  try {
    ...
    EsClient().execute(
      deleteIn(IndexAndType(s"${Settings.Mogopay.EsIndex}/BOTransaction"))
        .by("*"))
    EsClient().execute(
      deleteIn(IndexAndType(s"${Settings.Mogopay.EsIndex}/BOTransactionLog"))
        .by("*"))
    EsClient().execute(
      deleteIn(IndexAndType(s"${Settings.Mogopay.EsIndex}/ESSession")).by("*"))
    EsClient().execute(
      deleteIn(IndexAndType(s"${Settings.Mogopay.EsIndex}/TransactionSequence"))
        .by("*"))
    EsClient().execute(
      deleteIn(IndexAndType(s"${Settings.Mogopay.EsIndex}/TransactionRequest"))
        .by("*"))
  } catch {
